...
def cnn_learning(train_feats_proj, train_labels, test_feats_proj, test_labels,
                 hidden_dim=0, batch_size=0, learning_rate=0, epochs=0):
        '''
...
def deep_learning(train_feats_proj, train_labels, test_feats_proj, test_labels,
                  hidden_dim=64, num_layers=2, batch_size=128, learning_rate=0.001, epochs=50):

    # infer dims from data
    input_dim = int(train_feats_proj.shape[1])
    output_dim = int(np.max(train_labels) + 1)  # assumes labels are 0..C-1 (yours are 0..45)
...
def load_new_dataset(dataset_path, verbose=True,subject_index=9):
    ...
    if verbose:
            def print_set_info(name, f, l):
                ...
                print(f'--- {name} Set ---')
                print(f'\t# Classes: {len(classes)} | # Features: {f.shape[1]} | # Samples: {f.shape[0]}')
...
def example_classification(args, class_info):
    ...
    train_feats, train_labels, test_feats, test_labels = load_new_dataset(dataset_path=args.dataset_path, subject_index=9)
    ...
    plot_conf_mat(targets=train_labels, predictions=pred_train_labels, title="LDA Training Confusion Matrix (Unmerged)", 
                  filename="unmerged_train_confusion_matrix.png", class_names=class_info['names']['short_unmerged'], save_prefix="LDA")
    plot_conf_mat(targets=test_labels, predictions=pred_test_labels, title="LDA Testing Confusion Matrix (Unmerged)", 
                  filename="unmerged_test_confusion_matrix.png", class_names=class_info['names']['short_unmerged'], save_prefix="LDA")
    ...
    plot_conf_mat(targets=merged_train_labels, predictions=merged_pred_train_labels, title="LDA Training Confusion Matrix (Merged)",
                    filename="merged_train_confusion_matrix.png", class_names=class_info['names']['short_merged'], save_prefix="LDA")
    plot_conf_mat(targets=merged_test_labels, predictions=merged_pred_test_labels, title="LDA Testing Confusion Matrix (Merged)",
                    filename="merged_test_confusion_matrix.png", class_names=class_info['names']['short_merged'], save_prefix="LDA")
...
def classification():
   ...
   - Then find the average of all 10 subjects to get the final accuracy score
    '''
    train_feats, train_labels, test_feats, test_labels = load_new_dataset(dataset_path="Datasets/N_20_Takes_2.csv")
